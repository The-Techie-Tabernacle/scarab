...
class chooser(commands.Cog):
    ...
    async def first_region(self, is_minor: bool = False):
        async with self.connect_db() as db:
            ...
            async with db.execute(
                ...
                return Region(row['Name'], datetime.utcfromtimestamp(row[f'Last{"Minor" if is_minor else "Major"}Update']).astimezone(timezone.utc).time())
            ...
            async with db.execute(
                f'''
                    SELECT
                        Name, Last{"Minor" if is_minor else "Major"}Update
                    FROM Region WHERE
                    Last{"Minor" if is_minor else "Major"}Update <= (SELECT Last{"Minor" if is_minor else "Major"}Update FROM Region WHERE ID = ?) - ?
                    ORDER BY ID DESC
                    LIMIT 1
    ...
    async def select_targets(self,
                             ...
                             count: int = 1,
                             trigger_time: int = 4) -> TrigAndTargs:
        ...
        async with self.connect_db() as db:
            ...
            async with db.execute(
                    '''
                    SELECT
                        Name, LastMajorUpdate, LastMinorUpdate
                    FROM Region WHERE
                    DelegateAuth & 3 = 3 AND NOT hasPassword AND NOT DelegateVotes AND ID > ?
                    LIMIT ?
                    ''', (after_region, count)) as cursor:
                first_row = await cursor.fetchone()
                if not first_row:
                    last_region = await self.last_region(is_minor)
                    return TrigAndTargs(await self.select_trigger(last_region.name, trigger_time, is_minor), 0, [(last_region, 0)])
                first_target_name, first_target_update = first_row['Name'], first_row[f'Last{"Minor" if is_minor else "Major"}Update']
                targets = [(Region(first_target_name, datetime.utcfromtimestamp(first_target_update).astimezone(timezone.utc).time()), 0)]
                async for row in cursor:
                    targets.append(
                        (Region(
                            row['Name'],
                            datetime.utcfromtimestamp(row[f'Last{"Minor" if is_minor else "Major"}Update']).astimezone(timezone.utc).time()
        ...
        trigger_time = (datetime.combine(date.today(), trigger.last_update_time) -
                        datetime.combine(date.today(), datetime.utcfromtimestamp(first_target_update).astimezone(timezone.utc).time()))
    ...
    async def get_targets(self,
                          ...
                          trigger_time: int = 4):
        try:
            trigger, trigger_time, targets = await self.select_targets(is_minor, after_region, count, trigger_time)
